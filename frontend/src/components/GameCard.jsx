        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
          ...
          <div className="flex items-center justify-end gap-3 mt-2 interactive-area">
            ...
            <div className="relative" ref={menuRef}>
              ...
              {showMenu && (
                <div className="absolute right-0 top-8 bg-slate-900 border border-slate-700 shadow-xl rounded-lg z-20 w-48 overflow-hidden py-1">
                  ...
                  <div className="h-px bg-slate-800 my-1"></div>
                  <div className="px-3 py-2 text-xs font-semibold text-slate-500 uppercase border-b border-slate-800">Add to playlist</div>
                  {(playlists && playlists.length > 0) ? (
                    playlists.map(pl => (
                      <button
                        key={pl.id}
                        onClick={() => { onAddToPlaylist(pl.id, game); setShowMenu(false); }}
                        className="w-full text-left px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 truncate"
                      >
                        {pl.title}
                      </button>
                    ))
                  ) : (
                    <div className="px-3 py-2 text-xs text-slate-600">No playlists yet</div>
                  )}
                  <div className="h-px bg-slate-800 my-1"></div>
