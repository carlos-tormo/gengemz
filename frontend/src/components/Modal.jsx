...
const Modal = ({ isOpen, onClose, title, children, preventClose, containerClassName = "", contentClassName = "" }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200 ${containerClassName}`}>
      <div className={`bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] ${contentClassName}`}>
