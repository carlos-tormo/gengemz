...
import { Pencil, LayoutGrid, GripVertical } from 'lucide-react';
...
const Column = ({ column, games, onDragOver, onDrop, onDragStart, onMoveRequest, onDelete, isDraggingOver, filterPlatform, onHeaderClick, onEditColumn, onEditGame, onToggleFavorite }) => {
  const filteredItemIds = column.itemIds.filter(gameId => {
    if (!filterPlatform || filterPlatform === 'All') return true;
    const game = games[gameId];
    return game?.platform?.toLowerCase().includes(filterPlatform.toLowerCase());
  });
  ...
  return (
    ...
    >
      <div 
        ...
        className="p-4 flex items-center justify-between border-b border-slate-800/50 cursor-pointer group hover:bg-slate-800/50 rounded-t-xl transition-colors relative"
      >
        <div className="flex items-center gap-2 text-slate-200 font-bold tracking-wide">
          ...
          <span className="group-hover:text-white transition-colors">{column.title}</span>
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
           <button 
             ...
             className="p-1.5 hover:bg-slate-700 rounded-md text-slate-500 hover:text-white"
             title="Edit List"
           >
             <Pencil size={14} />
           </button>
           <LayoutGrid size={16} className="text-slate-600 ml-1" />
        </div>
      </div>

      <div className="p-3 flex-1 overflow-y-auto min-h-[150px] max-h-[calc(100vh-220px)] custom-scrollbar">
        {filteredItemIds.map((gameId, index) => {
          const game = games[gameId];
          if (!game) return null;
          return (
            <GameCard 
